...
describe("bikeName", () => {
  it("manda el nombre que le puso el ciclista", () => {
    // Si alguien bautizó su bici "La Roja", así se llama en todas las pantallas
    // y en el correo. Antes cada pantalla decidía por su cuenta.
    expect(bikeName({ name: "La Roja", brand: "Santa Cruz", model: "Hightower" }))
      .toBe("La Roja");
  });

  it("un nombre en blanco no cuenta como nombre", () => {
    expect(bikeName({ name: "   ", brand: "Yeti", model: "SB140" })).toBe("Yeti SB140");
  });

  it("sin nombre, junta marca y modelo", () => {
