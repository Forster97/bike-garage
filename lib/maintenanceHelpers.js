...
// Cuánto falta o cuánto se pasó, dicho como lo diría una persona.
// "vencida hace 12 días" no obliga a restar contra el calendario, que es lo que
// pedía "Próximo: 3 sep".
export function textoDeCuando({ status, last, remainingDays, remainingKm }) {
  if (!last && remainingDays == null) return "sin registro";

  if (remainingDays != null) {
    const d = Math.round(remainingDays);
    if (d < 0) return `vencida hace ${Math.abs(d)} ${Math.abs(d) === 1 ? "día" : "días"}`;
    if (d === 0) return "vence hoy";
    if (d <= 45) return `en ${d} ${d === 1 ? "día" : "días"}`;
    const m = Math.round(d / 30);
    return `en ${m} ${m === 1 ? "mes" : "meses"}`;
  }

  if (remainingKm != null) {
    const km = Math.round(remainingKm);
    if (km < 0) return `${Math.abs(km).toLocaleString("es-CL")} km pasados`;
    return `en ${km.toLocaleString("es-CL")} km`;
  }

  return status === "overdue" ? "vencida" : "al día";
}
